...
# Für Rückwärts-Kompatibilität (deprecated)
def get_prompt(mode: str, variant: str, text: str) -> str:
    """
    [DEPRECATED] Alte Methode für Single-Agent-Workflow.

    Verwende stattdessen get_prompt_multi_agent() für den neuen Multi-Agent-Workflow.
    """
    # Fallback auf kimi_k2 für ausformulieren, opus_analyse für korrekturlesen
    if mode == "ausformulieren":
        return get_prompt_multi_agent(mode, "kimi_k2", text)
    else:
        return get_prompt_multi_agent(mode, "opus_analyse", text)
